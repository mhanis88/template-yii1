...
$('.search-form form').submit(function(){
	$('#sys-module-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
...
<h3><?php echo Yii::t('app', 'Module') ?></h3>
...
if (GeneralFunction::roleCheckerAction(Yii::app()->user->role, Yii::app()->controller, "create") ||
    ...
    $header = '';
    if (GeneralFunction::roleCheckerAction(Yii::app()->user->role, Yii::app()->controller, 'create')):
        $header .= CHtml::link('<span class="glyphicon glyphicon-plus"></span>', Yii::app()->createUrl('config/sysModule/create'), array('class' => 'btn btn-info btn-sm', 'id' => 'add-record', 'data-toggle' => 'tooltip', 'data-placement' => 'top', 'title' => Yii::t('app', 'Add')));
    endif;
    ...
    if (GeneralFunction::roleCheckerAction(Yii::app()->user->role, Yii::app()->controller, 'deleteBulk')):
        ...
        $url = Yii::app()->createUrl('config/sysModule/deleteBulk');
        $ajaxOptions = [
            'type' => 'POST',
    ...
    $columns[] = [
        ...
        'buttons' => [
            'my_edit' => [
                ...
                'url' => 'Yii::app()->createUrl( "config/sysModule/update", ["id" => $data->module_id ] )',
